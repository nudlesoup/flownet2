...
if __name__ == '__main__':
    ...
    parser.add_argument('--flow_name', default='', type=str)
    ...
    with tools.TimerBlock("Parsing Arguments") as block:
        ...
        if args.inference:
            ...
            args.total_epochs = 1
            args.inference_dir = "{}/features".format(args.save)
    ...
    with tools.TimerBlock("Initializing Datasets") as block:
        ...
        if exists(args.inference_dataset_root):
            ...
            inference_loader = DataLoader(inference_dataset, batch_size=args.inference_batch_size, shuffle=False,
                                          **gpuargs)
    ...
    def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            data, target = [Variable(d, volatile=is_validate) for d in data], [Variable(t, volatile=is_validate) for t
                                                                               in target]
    ...
    def inference(args, epoch, data_loader, model, offset=0):
        ...
        if args.save_flow or args.render_validation:
            flow_folder = "{}/opticalflow_ego".format(args.inference_dir)
        ...
        for batch_idx, (data, target) in enumerate(progress):
            ...
            if args.save_flow or args.render_validation:
                for i in range(args.inference_batch_size):
                    ...
                    filename = args.flow_name
                    tt=batch_idx * args.inference_batch_size + i
                    u, v = _normalize_flow(_pflow)
                    uv = (np.dstack([u, v])*127.999+128).astype('uint8')
                    Image.fromarray(uv[...,0]).save( join(flow_folder, 'flow_x_%04d.jpg'.format(filename) % (batch_idx * args.inference_batch_size + i +1)))
                    Image.fromarray(uv[...,1]).save(join(flow_folder, 'flow_y_%04d.jpg'.format(filename) % (batch_idx * args.inference_batch_size + i + 1)))
                    # flow_utils.writeFlow(
                    #     join(flow_folder, '%04d.jpg'.format(filename) % (batch_idx * args.inference_batch_size + i)),tt,
                    #     _pflow)

            if args.inference_visualize:
                flow_utils.visulize_flow_file(
                    join(flow_folder, '{}-%04d.flo'.format(filename) % (batch_idx * args.inference_batch_size + i)),
                    flow_vis_folder)
    ...
    # Primary epoch loop
    best_err = 1e8
    progress = tqdm(range(args.start_epoch, args.total_epochs + 1), miniters=1, ncols=100, desc='Overall Progress',
