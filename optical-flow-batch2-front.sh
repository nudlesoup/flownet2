#!/bin/bash
for f in ~/Research/Chilli/train-batch-2/*; do
    if [ -d "$f" ]; then
        # $f is a directory
        #echo "$f"
        flow_file="${f##*/}"
        ...
        python mainflowxy-batch1front.py --inference --model FlowNet2 --save_flow --save $f/ --flow_name $flow_file --inference_dataset ImagesFromFolder --inference_dataset_root $f/synchronized/frames_front/ --resume pretrained/FlowNet2_checkpoint.pth.tar --number_gpus 1
